...
set _ftpCommandsFile=%~dp0\FtpCommandFiles\%backupType%WinscpDownloadCommands.txt

:: Those should not be changed
set _localFtpDirectory=%robotsBaseFolder%\%activeRobot%\%backupType%
...
if %normalizeSource%== true (CALL %~dp0\__normalizeSource.bat "__localFtpDirectory=%_localFtpDirectory%")
